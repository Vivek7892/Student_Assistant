...
var ABSOLUTE_URL_REGEX$12 = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i;
var isAbsoluteUrl2 = (url) => ABSOLUTE_URL_REGEX$12.test(url);
function resolvePath2(to, fromPathname) {
  ...
  let pathname;
  if (toPathname) {
    if (isAbsoluteUrl2(toPathname)) {
      pathname = toPathname;
    } else {
      if (toPathname.includes("//")) {
        let oldPathname = toPathname;
        toPathname = removeDoubleSlashes2(toPathname);
        warning2(false, "Pathnames cannot have embedded double slashes - normalizing " + (oldPathname + " -> " + toPathname));
      }
      if (toPathname.startsWith("/")) {
